...
func (h *UserMessageHandler) handle() error {
	if h.msg.IsText() {
		// _, err := h.msg.ReplyText(h.sender.ID())
		// if err != nil {
...
func (h *UserMessageHandler) ReplyText() error {
	...
	log.Printf("Received User[%v], UserID[%v], Content[%v], CreateTime[%v]", h.sender.NickName, h.sender.ID(), h.msg.Content,
	...
	// // 对话用户的ID打印到控制台
	// log.Println("user id:", h.sender.ID)

	// 2.向GPT发起请求，如果回复文本等于空,不回复
	// log.Println("requestText:", h.getRequestText())
	reply, err = gpt.ChatCompletions(h.getRequestText())
	if err != nil {
		text := err.Error()
	...
	log.Printf("Reply User[%v],Content[%v]", h.sender.NickName, reply)
	...
	if err != nil {
		return fmt.Errorf("reply user error: %v ", err)
		// } else {
		// 	// 3.如果回复成功，发送信息到callback_url 记录
		// 	if config.Config.CallbackURL != "" {
		// 		go func() {
		// 			err = h.service.SendCallbackMessage(requestText, reply)
		// 			if err != nil {
		// 				logger.Warning(fmt.Sprintf("send callback message error: %s", err))
		// 			}
		// 		}()
		// 	}
	}
