...
func (g *GroupMessageHandler) ReplyText() error {
	...
	log.Printf("Received Group[%v],User[%v], Content[%v], CreateTime[%v]", g.group.NickName, g.sender.NickName, g.msg.Content,
	...
	// 对话用户的ID打印到控制台
	log.Printf("Group[%v] User[%v] UserID[%v] RequestText[%v]", g.group.NickName, g.sender.NickName, g.sender.ID, requestText)
